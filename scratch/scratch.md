...
sequenceDiagram
    ...
    User->>ChatUI: Request operation<br/>(e.g., "add column")
    ChatUI->>Server: POST /api/messages
...
sequenceDiagram
    ...
    loop For each batch
        ...
        Note over LLM: 1. User's original intent
...
flowchart TD
  ...
  The error "violates foreign key constraint" indicates that the session_id\
  used while inserting into the messages table does not exist in the sessions table.
  A dummy or properly created session id must be used until authentication/session\
